...
//  CounterTests
//
//  Created by Oluwatobi Omotayo on 17/06/2022.
//

import XCTest
import ComposableArchitecture

// We are introducing this to distinguish between when we send an action
// and when we expect to receive an action from an effect.
enum StepType {
  case send
  case receive
}
...
func assert<Value: Equatable, Action: Equatable, Environment>(
  ...
  steps.forEach { step in
    var expected = state
    _ = reducer(&state, step.action, environment)
    ...
    case .receive:
      ...
      let effect = effects.removeFirst() // reducer(&state, step.action)
      var action: Action!
      let receivedExpectation = XCTestExpectation(description: "receivedExpectation")
      _ = effect.sink(
      ...
      if XCTWaiter.wait(for: [receivedExpectation], timeout: 0.01) != .completed {
